[**@mmutils/assert**](../../../../README.md)

***

[@mmutils/assert](../../../../modules.md) / [config/environment](../../README.md) / NodeJS

# NodeJS

## Classes

- [Immediate](classes/Immediate.md)
- [Timeout](classes/Timeout.md)

## Interfaces

- [AsyncIterator](interfaces/AsyncIterator.md)
- [CallSite](interfaces/CallSite.md)
- [CpuUsage](interfaces/CpuUsage.md)
- [Dict](interfaces/Dict.md)
- [EmitWarningOptions](interfaces/EmitWarningOptions.md)
- [ErrnoException](interfaces/ErrnoException.md)
- [EventEmitter](interfaces/EventEmitter.md)
- [HRTime](interfaces/HRTime.md)
- [Iterator](interfaces/Iterator.md)
- [MemoryUsage](interfaces/MemoryUsage.md)
- [MemoryUsageFn](interfaces/MemoryUsageFn.md)
- [Module](interfaces/Module.md)
- [Process](interfaces/Process.md)
- [ProcessConfig](interfaces/ProcessConfig.md)
- [ProcessEnv](interfaces/ProcessEnv.md)
- [ProcessFeatures](interfaces/ProcessFeatures.md)
- [ProcessPermission](interfaces/ProcessPermission.md)
- [ProcessRelease](interfaces/ProcessRelease.md)
- [ProcessReport](interfaces/ProcessReport.md)
- [ProcessVersions](interfaces/ProcessVersions.md)
- [ReadableStream](interfaces/ReadableStream.md)
- [ReadOnlyDict](interfaces/ReadOnlyDict.md)
- [ReadStream](interfaces/ReadStream.md)
- [ReadWriteStream](interfaces/ReadWriteStream.md)
- [RefCounted](interfaces/RefCounted.md)
- [Require](interfaces/Require.md)
- [~~RequireExtensions~~](interfaces/RequireExtensions.md)
- [RequireResolve](interfaces/RequireResolve.md)
- [RequireResolveOptions](interfaces/RequireResolveOptions.md)
- [ResourceUsage](interfaces/ResourceUsage.md)
- [Socket](interfaces/Socket.md)
- [Timer](interfaces/Timer.md)
- [WritableStream](interfaces/WritableStream.md)
- [WriteStream](interfaces/WriteStream.md)

## Type Aliases

- [Architecture](type-aliases/Architecture.md)
- [ArrayBufferView](type-aliases/ArrayBufferView.md)
- [BeforeExitListener](type-aliases/BeforeExitListener.md)
- [BufferEncoding](type-aliases/BufferEncoding.md)
- [BuiltinIteratorReturn](type-aliases/BuiltinIteratorReturn.md)
- [DisconnectListener](type-aliases/DisconnectListener.md)
- [ExitListener](type-aliases/ExitListener.md)
- [MessageListener](type-aliases/MessageListener.md)
- [MultipleResolveListener](type-aliases/MultipleResolveListener.md)
- [MultipleResolveType](type-aliases/MultipleResolveType.md)
- [Platform](type-aliases/Platform.md)
- [RejectionHandledListener](type-aliases/RejectionHandledListener.md)
- [Signals](type-aliases/Signals.md)
- [SignalsListener](type-aliases/SignalsListener.md)
- [TypedArray](type-aliases/TypedArray.md)
- [UncaughtExceptionListener](type-aliases/UncaughtExceptionListener.md)
- [UncaughtExceptionOrigin](type-aliases/UncaughtExceptionOrigin.md)
- [UnhandledRejectionListener](type-aliases/UnhandledRejectionListener.md)
- [WarningListener](type-aliases/WarningListener.md)
- [WorkerListener](type-aliases/WorkerListener.md)
